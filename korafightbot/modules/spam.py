...
async def spammer(event):
    if event.fwd_from:
        return
    kittu = await event.get_reply_message()
    kora = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    counter = int(kora[0])
    if counter > 50:
        sleeptimet = 0.5
        sleeptimem = 1
        ...
        sleeptimem = 0.3
    await event.delete()
    await spam_function(event, kittu, kora, sleeptimem, sleeptimet)


async def spam_function(event, kittu, kora, sleeptimem, sleeptimet, DelaySpam=False):
    hmm = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    counter = int(kora[0])
    if len(kora) == 2:
        spam_message = str(kora[1])
        for _ in range(counter):
            if event.reply_to_msg_id:
                await kittu.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and kittu.media:
        for _ in range(counter):
            kittu = await event.client.send_file(
                event.chat_id, kittu, caption=kittu.text
            )
            await _korautils.unsavegif(event, kittu)
            await asyncio.sleep(sleeptimem)
    elif event.reply_to_msg_id and kittu.text:
        spam_message = kittu.text
        for _ in range(counter):
            await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
        try:
        ...
        if BOTLOG:
            ...
            kittu = await event.client.send_file(BOTLOG_CHATID, kittu)
            await _korautils.unsavegif(event, kittu)
        return
    if BOTLOG:
        if DelaySpam is not True:
...
async def spammer(event):
    ...
    kora = input_str[1:]
    await event.delete()
    await spam_function(event, reply, kora, sleeptimem, sleeptimet, DelaySpam=True)
